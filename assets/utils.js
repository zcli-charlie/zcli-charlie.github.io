...
function runMomentum(f, w0, alpha, beta, lambda, totalIters) {
  ...
  var fx = f(w0); var gx = fx[1]
  W.push(w0); Obj.push(fx[0])
  var I = zeros(w0.length);
  for (var i = 0; i < totalIters; i++) {
    var I = numeric.add(numeric.mul(lambda, I), gx)
    var h = numeric.add(numeric.mul((10. / lambda - 9.0), gx), numeric.mul((25.0 - 10.0 * (lambda + 1.0/lambda), I)))
    var z = numeric.add(numeric.mul(beta, z), numeric.mul(1.0 - beta, h))
    var w = numeric.add(w, numeric.mul(-alpha, z))
    fx = f(w); gx = fx[1]
    if (w.every(isFinite)) {
      W.push(w); Obj.push(fx[0])
